...
-(NVCalendar *)createCalOfDay:(int)currentDay Month:(int)currentMonth Year:(int)currentYear MonthName:(NSString *)name
{
    ...
    UILabel *lblMonthName = [[UILabel alloc] initWithFrame:CGRectMake(0, 2, 144, 15)];
    ...
    lblMonthName.font = [UIFont fontWithName:@"HelveticaNeue-Medium" size:16.0];
    ...
    for (int d=1; d<=range.length;d++)
    {
        ...
        if (d-startWithDay>=0)
        {
            ...
            lblForDate.font = [UIFont fontWithName:@"HelveticaNeue-Thin" size:16.0];
            
            BOOL isCurrentDay = [self checkDate:add1DayDate];
            if(isCurrentDay)
            {
                lblForDate.backgroundColor = [UIColor colorWithRed:200.0/255.0 green:156.0/255.0 blue:120.0/255.0 alpha:1.0];
...
-(void)animationDrawCircleTime:(CGFloat)time label:(UILabel *)lbl completion:(void (^)(void))completion{
    ...
    circle.position =CGPointMake(lbl.frame.origin.x + 5,lbl.frame.origin.y);
    ...
    gradientLayer.startPoint=CGPointMake(1, 2.0);
...
-(void)createWeekDays:(int)d // for creating view of weekdays as well as weekends
{
    ...
    UILabel *lblNameOfDay = [[UILabel alloc] initWithFrame:CGRectMake(2*((d-1)*17), 22, day_label_width, day_label_height)];
    lblNameOfDay.text = [arrNames objectAtIndex:d-1];
    lblNameOfDay.font = [UIFont fontWithName:@"HelveticaNeue-Medium" size:15.0];
